...
from utils.evaluator import evaluate_ucp, evaluate_ucp_multi_stock
...
class Args:
    ...
    """if toggled, cost stock will be added to observations"""
    ...
    """list of initial cost stock values to evaluate with (e.g., [-25, -20, -15, -10, -5, 0]); if None, uses initial_cost_stock"""
...
class ContextualLagrangian(nn.Module):
    """
    Binned contextual Lagrangian multiplier λ(c₀).
    
    When initial cost stocks are sampled, each episode starts with a budget c₀. This
    module discretizes the configured c₀ range into bins and learns one multiplier per
    bin, so tighter budgets can receive stronger safety pressure.
    
    Args:
        c0_min: Minimum initial cost stock value
        c0_max: Maximum initial cost stock value
    ...
    def __init__(
        ...
        self.log_lambdas = nn.Parameter(
            torch.full((num_bins,), init_log, device=device, dtype=torch.float32)
        )
    
    def _c0_to_bin_idx(self, c0: torch.Tensor) -> torch.Tensor:
        """Convert c0 values to bin indices."""
        # Normalize c0 to [0, 1] range
    ...
    def forward(self, c0: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            c0: Initial cost stock values, shape (batch_size,) or (batch_size, 1)
            
        Returns:
            Lambda values, shape (batch_size, 1)
        """
        if c0.dim() == 1:
            c0 = c0.unsqueeze(-1)
        
        bin_idx = self._c0_to_bin_idx(c0.squeeze(-1))
        log_lambda = self.log_lambdas[bin_idx]
        lambda_val = F.softplus(log_lambda).unsqueeze(-1)
    ...
    def get_all_lambdas(self) -> torch.Tensor:
        """Return the learned lambda value for every bin."""
    ...
    def get_bin_centers(self) -> torch.Tensor:
        """Return bin center values for logging."""
        return torch.linspace(self.c0_min, self.c0_max, self.num_bins, device=self.device)
...
if __name__ == "__main__":
    ...
    augmented_wrapper = partial(
        ...
        cost_stock_min=args.cost_stock_min,
        cost_stock_max=args.cost_stock_max,
    ...
    envs = gym.vector.make(args.env_id, num_envs=args.num_envs, render_mode=render_mode, wrappers=[action_wrapper, augmented_wrapper], asynchronous=True) 
    
    cost_stock_index = envs.get_attr('cost_stock_index')[0]
    
    print(f"Observation augmentation configuration:")
    print(f"  Cost stock: {'enabled' if cost_stock_index is not None else 'disabled'} (index: {cost_stock_index})")
    if args.sample_initial_cost_stock:
        print(f"  Sample initial cost stock: enabled (range: [{args.cost_stock_min}, {args.cost_stock_max}])")
    ...
    def get_cost_q_params():
        # 1. Create one stateful cost critic to get initial parameters
        cost_qf = SoftQNetwork(envs, device=device, n_act=n_act, n_obs=n_obs, n_quantiles=args.n_quantiles,
                              hidden_sizes=args.critic_hidden_sizes,
                              use_embedding=args.use_embedding, embed_layers=args.embed_layers,
                              embed_connection_type=args.embed_connection_type,
                              embed_connection_layer=args.embed_connection_layer,
                              n_base_obs=n_base_obs, n_aug_features=n_aug_features)
        # 2. Extract its parameters into a functional container
        cost_qnet_params = from_module(cost_qf, as_module=True)
    ...
    if args.sample_initial_cost_stock:
        contextual_lagrangian = ContextualLagrangian(
            c0_min=args.cost_stock_min,
            c0_max=args.cost_stock_max,
        ...
        lagrangian_multiplier = torch.nn.Parameter(
            torch.as_tensor(max(float(args.lagrangian_multiplier_init), 0.0), device=device),
            requires_grad=True,
        )
    ...
    def batched_cost_qf(params, obs, action):
        """Returns quantiles for cost Q-function."""
        with params.to_module(cost_qnet):
            return cost_qnet(obs, action)
    ...
    def update_main(data):
        ...
        Cost Critic (Distributional Bellman for costs with n-step):

        $$L_C = \mathbb{E}_{(s,a,c^{(n)},s_{t+n}) \sim \mathcal{D}} \left[ \sum_{i=1}^N \rho_{\tau_i}(\delta^c_i) \right]$$

        where:
        $$\delta^c_i = c^{(n)} + \gamma^n (1 - d) \cdot Z^c_{\theta'}(s_{t+n}, \tilde{a}')_{\tau_i} - Z^c_\theta(s, a)_{\tau_i}$$
        ...
        with torch.no_grad():
            ...
            cost_next_quantiles = batched_cost_qf(cost_qnet_target, next_obs, next_state_actions)
            ...
            # Cost critic targets (also distributional with n-step)
            # next_costs already contains the n-step accumulated costs from MultiStepTransform
            target_cost_quantiles = next_costs.unsqueeze(-1) + (~next_done.unsqueeze(-1)).float() * gamma_n.unsqueeze(-1) * cost_next_quantiles
        ...
        # Cost critic loss (quantile regression)
        current_cost_quantiles = batched_cost_qf(cost_qnet_params, data["observations"], data["actions"])
        cost_loss, _ = compute_quantile_regression_loss(current_cost_quantiles, target_cost_quantiles)
        cost_loss.backward()
        ...
        with torch.no_grad():
            ...
            expected_cost_q = current_cost_quantiles.mean(dim=1).mean()
    ...
    cost_stock_idx_val = cost_stock_index if use_cost_stock else -1  # -1 as placeholder
    ...
    def update_pol(data, constants):
        ...
        Cost Q-value (stock-aware if enabled):
        - With cost stock:
          $$U^c(s, a) = \mathbb{E}_{\tau}[\max(0, Z^c_\theta(s, a)_\tau + \text{stock}_c \cdot \text{normalizer}_c)]$$
        - Without cost stock:
          $$U^c(s, a) = \mathbb{E}_{\tau}[Z^c_\theta(s, a)_\tau]$$

        SAC-style actor loss:
        $$L_\pi = \mathbb{E}_{s \sim \mathcal{D}} \left[
            \frac{\alpha \log \pi(\tilde{a}|s) - U(s,\tilde{a}) + \lambda U^c(s,\tilde{a}) + L_{\text{reg}}(s)}
                 {1 + \lambda}
        \right]$$
        ...
        - $\lambda$ = Lagrange multiplier (learned to enforce $J^c \leq d$)
        """
        ...
        cost_q_pi_quantiles = batched_cost_qf(cost_qnet_params, data["observations"], pi)
        
        if use_cost_stock:
            # Extract cost stock from observations using the stored index
            cost_stock = data["observations"][:, cost_stock_idx_val].unsqueeze(-1)  # (batch_size, 1)
            cost_stock_value = constants["cost_normalizer"] * cost_stock  # (batch_size, 1)
            
            # Add cost stock to quantiles and apply utility function
            if args.cost_utility_type == "abs":
                cost_utility_quantiles = cost_q_pi_quantiles + cost_stock_value  # (batch_size, n_quantiles)
            ...
            cost_q_pi = cost_utility_quantiles.mean(dim=-1, keepdim=True)
        else:
            # Standard cost Q-value without stock
            cost_q_pi = cost_q_pi_quantiles.mean(dim=-1, keepdim=True)
        
        # Get Lagrangian multiplier (scalar or contextual)
        if "contextual_lagrangian" in constants.keys():
            ...
            c0 = data["initial_cost_stock"]  # (batch_size,)
            lagrangian_multiplier_per_sample = constants["contextual_lagrangian"](c0).detach()  # (batch_size, 1)
            lagrangian_multiplier_mean = lagrangian_multiplier_per_sample.mean()
        else:
            # Scalar λ: same for all samples
            lagrangian_multiplier_per_sample = constants["lagrangian_multiplier"]  # scalar
            lagrangian_multiplier_mean = lagrangian_multiplier_per_sample
        ...
        if "contextual_lagrangian" in constants.keys():
            ...
            per_sample_cost = lagrangian_multiplier_per_sample * cost_q_pi  # (batch_size, 1)
            
            numerator = per_sample_policy_loss + per_sample_reg + per_sample_cost
            denominator = 1 + lagrangian_multiplier_per_sample
            actor_loss = (numerator / denominator).mean()
        else:
            policy_loss = (alpha.detach() * log_pi - qf_pi).mean()
            actor_loss = (policy_loss + policy_reg_loss + lagrangian_multiplier_per_sample * cost_q_pi.mean()) / (1 + lagrangian_multiplier_per_sample)
    ...
    ep_costs = np.zeros(envs.num_envs, dtype=np.float32)
    ...
    discounted_ep_costs = np.zeros(envs.num_envs, dtype=np.float32)
    ...
    if args.sample_initial_cost_stock and cost_stock_index is not None:
        # Initialize with the initial observation's cost stock (already normalized)
        initial_cost_stocks = obs[:, cost_stock_index].cpu().numpy() * args.cost_normalizer
    else:
        initial_cost_stocks = np.full(envs.num_envs, args.initial_cost_stock, dtype=np.float32)
    ...
    constants = TensorDict({
        "cost_normalizer": torch.tensor(args.cost_normalizer, device=device),
    ...
    for global_step in pbar:
        ...
        for idx in done_indices:
            ...
            # Compute objective using appropriate lambda
            if args.sample_initial_cost_stock and contextual_lagrangian is not None:
                c0_tensor = torch.tensor([initial_cost_stocks[idx]], device=device)
                lambda_val = contextual_lagrangian(c0_tensor).item()
            else:
                lambda_val = lagrangian_multiplier.item()
            avg_objective.append(ep_return - lambda_val * ep_cost)
            max_ep_ret = max(max_ep_ret, ep_return)
            max_ep_cost = max(max_ep_cost, ep_cost)
            
            # Store the completed trajectory for the next contextual λ update.
            if args.sample_initial_cost_stock and contextual_lagrangian is not None:
                trajectory_cost_buffer.append((initial_cost_stocks[idx], ep_cost))
        ...
        if avg_returns:
            ...
            mean_cost = float(np.mean(avg_costs)) if avg_costs else 0.0
            ...
            desc = (
                ...
                f"cost={mean_cost: 4.2f} (max={max_ep_cost: 4.2f}), "
        ...
        # The next observation for reset environments contains the new initial cost stock
        if args.sample_initial_cost_stock and cost_stock_index is not None:
            for idx in done_indices:
                # Get the new initial cost stock from next_obs (which is now the reset observation)
                initial_cost_stocks[idx] = next_obs[idx, cost_stock_index].item() * args.cost_normalizer
        ...
        if global_step > args.learning_starts and data is not None:
            ...
            if avg_costs and global_step > max(args.learning_starts, args.lagrange_warmup_steps):
                """
                Lagrange Multiplier Update for Safety Constraint:
                
                For scalar λ:
                $$\lambda_{k+1} = \text{proj}_{[0, \infty)} \left[ \lambda_k + \eta_\lambda \cdot (J^c(\pi_k) - d) \right]$$
                
                For contextual λ(c₀):
                The constraint is: episodic_cost + c₀ ≤ 0 (i.e., cost_stock_final ≤ 0)
                The violation is: episodic_cost + c₀
                We use actual trajectory costs to update λ(c₀) for each completed trajectory.
                """
                if args.sample_initial_cost_stock and contextual_lagrangian is not None:
                    # Update each active bin from recently completed trajectories.
                    if len(trajectory_cost_buffer) > 0:
                        # Convert buffer to tensors
                        c0_list, ep_cost_list = zip(*trajectory_cost_buffer)
                        c0_batch = torch.tensor(c0_list, device=device, dtype=torch.float32)
                        ep_cost_batch = torch.tensor(ep_cost_list, device=device, dtype=torch.float32)
                        
                        # Get λ(c₀) for each trajectory's initial cost stock.
                        lambda_vals = contextual_lagrangian(c0_batch)  # (n_trajectories, 1)
                        ...
                        if args.lagrangian_utility_type == "abs":
                            ...
                            violation = torch.clamp(ep_cost_batch + c0_batch, min=0.0).unsqueeze(-1) - args.lagrangian_utility_epsilon  # (n_trajectories, 1)
                            ...
                            violation = (ep_cost_batch + c0_batch).unsqueeze(-1)  # (n_trajectories, 1)
                    ...
                    # Scalar Lagrangian update
                    if args.lagrangian_utility_type == "abs":
                        violation = np.mean(np.maximum(args.initial_cost_stock + np.array(avg_costs), 0.0)) - args.lagrangian_utility_epsilon
                    elif args.lagrangian_utility_type == "mean":
                        violation = np.mean(args.initial_cost_stock + np.array(avg_costs))
                    
                    lambda_loss = -lagrangian_multiplier * violation
                    lambda_optimizer.zero_grad()
                    lambda_loss.backward()
                    lambda_optimizer.step()
            ...
            if global_step % 100 == 0 and start_time is not None:
                ...
                with torch.no_grad():
                    # Get lagrange multiplier for logging (scalar or mean of contextual)
                    if args.sample_initial_cost_stock and contextual_lagrangian is not None:
                        ...
                        lambda_for_log = lagrangian_multiplier.item()
                    
                    logs = {
                        "charts/episodic_return": torch.tensor(list(avg_returns)).mean() if avg_returns else torch.tensor(0.0),
                        "charts/episodic_cost": torch.tensor(list(avg_costs)).mean() if avg_costs else torch.tensor(0.0),
                        ...
                        "charts/discounted_cost": torch.tensor(list(avg_discounted_costs)).mean() if avg_discounted_costs else torch.tensor(0.0),
                    ...
                    if args.use_tb:
                        ...
                        # Log per-bin lambda values for contextual Lagrangian
                        if args.sample_initial_cost_stock and contextual_lagrangian is not None:
                            all_lambdas = contextual_lagrangian.get_all_lambdas()
                            bin_centers = contextual_lagrangian.get_bin_centers()
                            for i, (c0, lam) in enumerate(zip(bin_centers.tolist(), all_lambdas.tolist())):
                                writer.add_scalar(f"contextual_lambda/c0_{c0:.1f}", lam, global_step)
        ...
        if args.save_model and global_step % args.save_freq == 0 and global_step > 0:
            ...
            checkpoint_dict = {
                ...
                'lagrangian_multiplier': lagrangian_multiplier.item() if lagrangian_multiplier is not None else contextual_lagrangian.get_all_lambdas().mean().item(),
    ...
    if args.evaluation_episodes > 0 and args.save_model:
        ...
        if args.eval_initial_cost_stocks is not None and len(args.eval_initial_cost_stocks) > 0:
            ...
            eval_results = evaluate_ucp_multi_stock(
                model_save_path,
            ...
            eval_results = evaluate_ucp(model_save_path, args.evaluation_episodes, args.evaluation_temperature)
